...
public class BuddingOpalBlock extends OpalBlock {
    ...
    public BuddingOpalBlock(Properties properties) {
        super(properties);
    }
    ...
    public void randomTick(BlockState blockState, ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSource) {
        if (randomSource.nextInt(5) == 0) {
            ...
            if (canClusterGrowAtState(blockstate)) {
                block = ModBlocks.SMALL_OPAL_BUD.get(); // Blocks.SMALL_AMETHYST_BUD;
            } else if (blockstate.is(ModBlocks.SMALL_OPAL_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlocks.MEDIUM_OPAL_BUD.get(); // Blocks.MEDIUM_AMETHYST_BUD;
            } else if (blockstate.is(ModBlocks.MEDIUM_OPAL_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlocks.LARGE_OPAL_BUD.get(); // Blocks.LARGE_AMETHYST_BUD;
            } else if (blockstate.is(ModBlocks.LARGE_OPAL_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlocks.OPAL_CLUSTER.get(); // Blocks.AMETHYST_CLUSTER;
            }
